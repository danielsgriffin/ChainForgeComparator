/**
 * An inspector UI for examining Pairwise Comparison responses (adapted from PairwiseComparisonInspector).
 * 
...
import { IconTable, IconLayoutList, IconTrophy } from '@tabler/icons-react';
...
const getEvalResultStr = (eval_item) => {
  if (Array.isArray(eval_item)) {
    ...
    return (<>
      <span style={{color: 'gray'}}>{"selected response: "}</span>
      <span style={{color: color}}>System {eval_str.toUpperCase()}</span>
    </>);
  }
};
...
const PairwiseComparisonInspector = ({ jsonResponses, wideFormat }) => {
  ...
  useEffect(() => {
    ...
    // Process responses to calculate scores and matches
    let scoreCounter = {};
    let matchSummaries = [];

    jsonResponses.forEach(res_obj => {
      const systemA = res_obj.metavars.system_A;
      const systemB = res_obj.metavars.system_B;
      const evalResults = res_obj.eval_res ? res_obj.eval_res.items : [];

      evalResults.forEach(evalItem => {
        const evalStr = evalItem.toString().trim().toLowerCase();
        const winner = evalStr === 'a' ? systemA : (evalStr === 'b' ? systemB : null);
        if (winner) {
          scoreCounter[winner] = (scoreCounter[winner] || 0) + 1;
        }
        matchSummaries.push({ systemA, systemB, winner });
      });
    ...
    // Convert scoreCounter to array suitable for table display
    const scoreTableData = Object.entries(scoreCounter).map(([system, score]) => ({ system, score }));
    setScores(scoreTableData);
    ...
    const generateResponseBoxes = (resps, eatenvars, fixed_width) => {
      return resps.map((res_obj, res_idx) => {
        const eval_res_items = res_obj.eval_res ? res_obj.eval_res.items : null;
        // Extract the key variables from the metavars
        const system_A = res_obj.metavars.system_A;
        const system_B = res_obj.metavars.system_B;
        const system_A_text = res_obj.metavars.system_A_text;
        const system_B_text = res_obj.metavars.system_B_text;
        ...
        const ps = same_resp_keys.map((r, idx) => (
          <div key={idx}>
            ...
            {(contains_eval_res && onlyShowScores) ? <pre>{}</pre> :  
              <><pre className="small-response"><strong>System A: {system_A}</strong><br></br>{system_A_text}</pre><pre className="small-response"><strong>System B: {system_B}</strong><br></br>{system_B_text}</pre></>
            }
          </div>
        ));
        ...
        return (
            <div key={"r"+res_idx} className="response-box" style={{ backgroundColor: color_for_llm(getLLMName(res_obj)), width: `${fixed_width}%` }}>
                ...
                {eatenvars.includes('LLM') ?
                      ps
                    : (<div className="comparison-inspector-prompt-text">
                        <h1>{res_obj.prompt}</h1>
                        {ps}
                      </div>)
                }
            </div>
  ...
  return (<div style={{height: '100%'}}>
    ...
    <Tabs value={viewFormat} onTabChange={setViewFormat} styles={{tabLabel: {fontSize: wideFormat ? '12pt' : '9pt' }}}>
      <Tabs.List>
        ...
        <Tabs.Tab value="scores"><IconTrophy size="10pt" style={{ marginBottom: wideFormat ? '0px' : '-4px' }} />{wideFormat ? " Scores" : ""}</Tabs.Tab>
      </Tabs.List>
            ...
            <tbody>
              {matches.map((match, index) => (
                <tr key={index}>
                  <td>{match.systemA}</td>
                  <td>{match.systemB}</td>
                  <td>{match.winner || 'Tie/No Winner'}</td>
                </tr>
              ))}
            </tbody>
          </Table>
...
export default PairwiseComparisonInspector;