...
import PairwiseComparisonInspector, { exportToExcel } from './PairwiseComparisonInspector';

const PairwiseComparisonInspectorModal = forwardRef((props, ref) => {
  ...
  return (
    <Modal size='90%' keepMounted opened={opened} onClose={close} closeOnClickOutside={true} style={{position: 'relative', 'left': '-5%'}} title={
      <div><span>Pairwise Comparison Inspector</span><button className="custom-button" style={{marginTop: 'auto', marginRight: '14px', float: 'right'}} onClick={() => exportToExcel(props.jsonResponses)}>Export data to Excel</button></div>
      ...
      <div className="inspect-modal-response-container" style={{padding: '6px', overflow: 'scroll'}}>
        <PairwiseComparisonInspector jsonResponses={props.jsonResponses} wideFormat={true} />
      </div>
    </Modal>
  );
});

export default PairwiseComparisonInspectorModal;